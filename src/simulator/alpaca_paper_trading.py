...
def get_next_market_open():
    ...
    Returns:
        datetime.datetime: Next market open time in Central Timezone
    """
    try:
        clock = api.get_clock()
        next_open = clock.next_open
        # Convert to Central Time
        next_open_ct = next_open.astimezone(CENTRAL_TIMEZONE)
        return next_open_ct
...
def is_market_open() -> bool:
    """
    Check if the market is currently open or within 30 minutes of opening.
    
    Returns:
        bool: True if market is open or within 30 minutes of opening, False otherwise
    """
    try:
        clock = api.get_clock()
        if clock.is_open:
        ...
        # Check if we're within 30 minutes of market open
        next_open = get_next_market_open()
        if next_open:
            now = datetime.datetime.now(CENTRAL_TIMEZONE)
            time_to_open = (next_open - now).total_seconds() / 60  # Convert to minutes
            return time_to_open <= 30
        ...
        for day_data in pattern_data:
            ...
            for session in ['morning', 'mixed', 'afternoon']:
                ...
                for pattern in session_patterns:
                    ...
                    # Override TP and SL to 1 and 2 (SPY terms)
                    spy_tp = 1.0
                    spy_sl = 2.0
...
def main():
    ...
    # Check if market is open or within 30 minutes of opening
    if not is_market_open():
        next_open = get_next_market_open()
        if next_open:
            print(f"Market is currently closed. Next market open is at {next_open.strftime('%Y-%m-%d %H:%M:%S')} CT")
            print("Please run this script during market hours or within 30 minutes of market open.")
        else:

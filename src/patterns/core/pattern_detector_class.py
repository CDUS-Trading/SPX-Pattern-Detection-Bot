...
class PatternDetector:
        ...
        try:
            ...
            for pattern in self.patterns:
                ...
                if (range1_min <= metric1 <= range1_max and 
                    ...
                    # Add matched metric values to the pattern dict
                    pattern = pattern.copy()  # avoid mutating the original
                    pattern['matched_metric_values'] = {
                        pattern1: metric1,
                        pattern2: metric2
                    }
    ...
    def generate_trading_strategy(self, pattern: Dict) -> Dict:
        ...
        try:
            ...
            # Add pattern_dates if present
            if 'pattern_dates' in pattern:
                strategy['pattern_dates'] = pattern['pattern_dates']

...
TP_VALUE = 10  # Take Profit in points
SL_VALUE = 20  # Stop Loss in points

# --- Load mapping of timeframes to minute indices from JSON ---
...
def load_timeframe_indices():
    with open(TRANSLATORY_CONSTANTS_PATH, 'r') as f:
        constants = json.load(f)
    return {k: tuple(v) for k, v in constants['timeframe_indices'].items()}

TIMEFRAME_INDICES = load_timeframe_indices()

def get_timeframe_indices(timeframe: str) -> Tuple[int, int]:
    if timeframe not in TIMEFRAME_INDICES:
        raise ValueError(f"Unknown or unsupported timeframe: {timeframe}")
    return TIMEFRAME_INDICES[timeframe]

def vectorized_tp_sl_hit(tf_minutes, entry, direction, tp, sl):
    if direction == 'bullish':
        tp_hit = tf_minutes['high'] >= entry + tp
        sl_hit = tf_minutes['low'] <= entry - sl
    else:  # bearish
        tp_hit = tf_minutes['low'] <= entry - tp
        sl_hit = tf_minutes['high'] >= entry + sl

    # Find the first index where TP or SL is hit
    tp_idx = np.where(tp_hit)[0]
    ...
    if len(tp_idx) == 0 and len(sl_idx) == 0:
        ...
        # Both are hit, check which comes first
...
def parse_arguments() -> argparse.Namespace:
    ...
    try:
        ...
        today_minus_3_months = (datetime.now() - pd.DateOffset(months=3)).strftime('%Y-%m-%d')
        parser.add_argument('--input-file', type=str, default='data/SPX_full_1min.txt', help='Input file path (default: data/SPX_full_1min.txt)')
        parser.add_argument('--debug', action='store_true', help='Enable debug logging')
        parser.add_argument('--output-file', type=str, default=None, help='Output file name (if not provided, uses versioning system)')
        parser.add_argument('--start-date', type=str, default='2022-10-01', help='Start date for pattern collection (YYYY-MM-DD). Default: 2022-10-01')
        parser.add_argument('--end-date', type=str, default=today_minus_3_months, help='End date for pattern collection (YYYY-MM-DD). Default: today minus 3 months')
...
def analyze_market_correlation(minute_file: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        end_date (str, optional): End date for analysis (YYYY-MM-DD). Defaults to None (use all data up to last date).
    ...
    try:
        ...
        df = pd.read_csv(minute_file, parse_dates=['datetime'])
        df.set_index('datetime', inplace=True)
        
        # Filter for last 3 years or from start_date
        last_date = df.index.max()
        if start_date:
            try:
                start_dt = pd.to_datetime(start_date)
            except Exception:
                raise DataError(f"Invalid start date format: {start_date}. Use YYYY-MM-DD.")
        else:
            start_dt = last_date - pd.DateOffset(years=3)
        
        if end_date:
            try:
                end_dt = pd.to_datetime(end_date)
            except Exception:
                raise DataError(f"Invalid end date format: {end_date}. Use YYYY-MM-DD.")
        else:
            end_dt = last_date
        
        df = df[(df.index >= start_dt) & (df.index <= end_dt)]
        
        print(f"Number of rows after date filtering: {len(df)}")
...
def analyze_patterns(df: pd.DataFrame, minute_data_by_date: dict) -> List[Dict]:
    """
    ...
    Args:
        ...
        minute_data_by_date (dict): Dict mapping date to minute-level DataFrame for that day
    ...
    with open(output_file, 'w') as f:
        ...
        for pattern1, pattern2 in tqdm(pattern_combinations, desc="Analyzing pattern combinations"):
            ...
            patterns = analyze_combined_pattern(df, pattern1, pattern2, minute_data_by_date)
...
def analyze_combined_pattern(df: pd.DataFrame, pattern1: str, pattern2: str, minute_data_by_date: dict) -> List[Dict]:
    """
    ...
    patterns = []
    # Remove hardcoded timeframes, fetch from TIMEFRAME_INDICES
    timeframes = list(TIMEFRAME_INDICES.keys())
            ...
            if len(subset) > 0:
                ...
                for timeframe in timeframes:
                    ...
                    if col not in subset.columns:
                        continue
                    # --- Combined direction and TP/SL hit simulation ---
                    bullish_first = 0
                    bearish_first = 0
                    bullish_tp = 0
                    bullish_sl = 0
                    ...
                    for _, row in subset.iterrows():
                        ...
                        try:
                            start_idx, end_idx = get_timeframe_indices(timeframe)
                        except Exception:
                            continue
                        day_minutes = minute_data_by_date.get(date)
                        if day_minutes is None or len(day_minutes) < end_idx:
                            continue
                        tf_minutes = day_minutes.iloc[start_idx:end_idx]
                        if tf_minutes.empty:
                            continue
                        entry = tf_minutes.iloc[0]['open']
                        # Use vectorized function for both directions
                        bull_result = vectorized_tp_sl_hit(tf_minutes, entry, 'bullish', TP_VALUE, SL_VALUE)
                        bear_result = vectorized_tp_sl_hit(tf_minutes, entry, 'bearish', TP_VALUE, SL_VALUE)
                        # Bullish
                        if bull_result == 'tp':
                            bullish_tp += 1
                        elif bull_result == 'sl':
                            ...
                            bullish_none += 1
                        # Bullish success: SL not hit at any time, and close > entry
                        sl_hit_any_bull = (tf_minutes['low'] <= entry - SL_VALUE).any()
                        if not sl_hit_any_bull and tf_minutes.iloc[-1]['close'] > entry:
                            bullish_success += 1
                        # Bearish
                        if bear_result == 'tp':
                            bearish_tp += 1
                            ...
                            bearish_none += 1
                        # Bearish success: SL not hit at any time, and close < entry
                        sl_hit_any_bear = (tf_minutes['high'] >= entry + SL_VALUE).any()
                        if not sl_hit_any_bear and tf_minutes.iloc[-1]['close'] < entry:
                            bearish_success += 1
                        # For direction: which is hit first?
                        hit_dir = None
                        if bull_result == 'tp' and bear_result != 'tp':
                            hit_dir = 'bullish'
                        elif bear_result == 'tp' and bull_result != 'tp':
                            hit_dir = 'bearish'
                        elif bull_result == 'tp' and bear_result == 'tp':
                            # If both hit TP, pick the one that happens first in time
                            bull_tp_idx = np.where(tf_minutes['high'] >= entry + TP_VALUE)[0]
                            bear_tp_idx = np.where(tf_minutes['low'] <= entry - TP_VALUE)[0]
                            if len(bull_tp_idx) > 0 and len(bear_tp_idx) > 0:
                                hit_dir = 'bullish' if bull_tp_idx[0] <= bear_tp_idx[0] else 'bearish'
                        if hit_dir == 'bullish':
                            bullish_first += 1
                        elif hit_dir == 'bearish':
                            bearish_first += 1
                        else:
                            if tf_minutes.iloc[-1]['close'] > entry:
                                bullish_first += 1
                            else:
                                bearish_first += 1
                    # Decide direction
                    is_bearish = bearish_first > bullish_first
                    # Use the correct TP/SL stats for the chosen direction
                    if is_bearish:
                        tp_first = bearish_tp
                        sl_first = bearish_sl
                        total = bearish_tp + bearish_sl + bearish_none
                        success_count = bearish_success
                    else:
                        tp_first = bullish_tp
                        sl_first = bullish_sl
                        total = bullish_tp + bullish_sl + bullish_none
                        success_count = bullish_success
                    # --- Existing stats logic (unchanged) ---
                    returns = subset[col]
                    if is_bearish:
                        expected_moves = returns[returns < 0]
                        opposite_moves = returns[returns > 0]
                    else:
                        expected_moves = returns[returns > 0]
                        opposite_moves = returns[returns < 0]
                    min_move_threshold = 0
                    if len(expected_moves) > 0 and all(abs(move) >= min_move_threshold for move in expected_moves):
                        avg_expected_move = abs(expected_moves.mean())
                        ...
                        # --- Favorable/Adverse move statistics ---
                        favorable_avg_move = expected_moves.mean()
                        favorable_median_move = expected_moves.median()
                        favorable_std = expected_moves.std()
                        adverse_avg_move = opposite_moves.mean() if len(opposite_moves) > 0 else 0
                        adverse_median_move = opposite_moves.median() if len(opposite_moves) > 0 else 0
                        adverse_std = opposite_moves.std() if len(opposite_moves) > 0 else 0
                    else:
                        continue
                    cat1_str = str(cat1)
                    cat2_str = str(cat2)
                    ...
                    # Store raw counts instead of percentages
                    patterns.append({
                        'pattern1': pattern1,
                        'cat1': cat1,
                        'pattern2': pattern2,
                        ...
                        'fav_direction_count': bullish_success if is_bearish else bearish_success,
                        'fav_direction_rate': (bullish_success if is_bearish else bearish_success) / total,
                        'tp_hit_count': tp_first,
                        'tp_hit_rate': tp_first / total,
                        'sl_hit_count': sl_first,
                        'sl_hit_rate': sl_first / total,
                        'risk_reward': risk_reward,
                        'avg_win': avg_expected_move,
                        'avg_loss': avg_opposite_move,
                        # Favorable/adverse move stats
                        'favorable_avg_move': favorable_avg_move,
                        'favorable_median_move': favorable_median_move,
                        'favorable_std': favorable_std,
                        'adverse_avg_move': adverse_avg_move,
                        'adverse_median_move': adverse_median_move,
                        'adverse_std': adverse_std
                    })
    return patterns
...
def save_patterns_to_json(patterns: List[Dict], output_file: str) -> List[Dict]:
    ...
    for pattern in patterns:
        ...
        # Extract dates from the subset
        pattern_dates = []
        if 'subset' in pattern and 'date' in pattern['subset']:
            dates = pattern['subset']['date']
            if isinstance(dates, pd.Series):
                pattern_dates = [d.strftime('%Y-%m-%d') if isinstance(d, pd.Timestamp) else str(d) for d in dates]
            else:
        ...
        pattern_entry = {
            ...
            'fav_direction_count': int(pattern['fav_direction_count']),
            'fav_direction_rate': float(pattern['fav_direction_rate']),
            ...
            'tp_hit_count': int(pattern['tp_hit_count']),
            'tp_hit_rate': float(pattern['tp_hit_rate']),
            'sl_hit_count': int(pattern['sl_hit_count']),
            'sl_hit_rate': float(pattern['sl_hit_rate']),
            # Favorable/adverse move stats
            'favorable_avg_move': float(pattern.get('favorable_avg_move', 0)),
            'favorable_median_move': float(pattern.get('favorable_median_move', 0)),
            'favorable_std': float(pattern.get('favorable_std', 0)),
            'adverse_avg_move': float(pattern.get('adverse_avg_move', 0)),
            'adverse_median_move': float(pattern.get('adverse_median_move', 0)),
            'adverse_std': float(pattern.get('adverse_std', 0)),
...
def main() -> None:
    ...
    df = analyze_market_correlation(full_filepath, start_date=args.start_date, end_date=args.end_date)
    # Load minute data for TP/SL analysis
    minute_df = pd.read_csv(full_filepath, parse_dates=['datetime'])
    minute_df['date_only'] = minute_df['datetime'].dt.date
    minute_data_by_date = {date: df for date, df in minute_df.groupby('date_only')}
    patterns = analyze_patterns(df, minute_data_by_date)
    ...
    if args.output_file:
        ...
        output_file = os.path.join(processed_dir, f"master_pattern_database_v{next_version}_{timestamp}.json")

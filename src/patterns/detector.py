...
def parse_arguments() -> argparse.Namespace:
    ...
    try:
        ...
        parser.add_argument('--show-dates-list', '-sdl', action='store_true', help='Show historical dates for each pattern as a side-by-side list in the TXT output')
...
def print_analysis(metrics: Dict[str, float], matched_patterns: List[Dict], target_date: datetime.date,
    ...
    try:
        ...
        # Convert target_date to string format for comparison
        target_date_str = target_date.strftime('%Y-%m-%d')
        # Get the last close price for the target date
        prev_close = analysis_data['close'].iloc[-1]
        ...
        if matched_patterns:
            ...
            for pattern in matched_patterns:
                ...
                # Store both strategy and the original pattern
                patterns_by_period[period].append((strategy, pattern))
            ...
            for period in ['morning', 'mixed', 'afternoon']:
                if period in patterns_by_period:
                    ...
                    # Unpack strategies for filtering
                    strategies = [sp[0] for sp in patterns_by_period[period]]
                    highest_prob_patterns = filter.filter_by_highest_probability(strategies)
                    ...
                    for strategy in filtered_patterns:
                        try:
                            # Find the original pattern for this strategy
                            orig_tuple = next((sp for sp in patterns_by_period[period] if sp[0] == strategy), None)
                            orig_pattern = orig_tuple[1] if orig_tuple else None
                            ...
                            if strategy['direction'] == 'bullish':
                                ...
                                tp_points = prev_close * (tp_pct / 100)
                                sl_points = prev_close * (sl_pct / 100)
                            # Get historical dates for this pattern
                            historical_dates = strategy.get('pattern_dates', [])
                            # Use matched_metric_values from the original pattern
                            matched_metric_values = orig_pattern.get('matched_metric_values', {}) if orig_pattern else {}
                            pattern_data = {
                                ...
                                "matched_metric_values": matched_metric_values  # Add actual metric values
                            }
        ...
        # Save current patterns to TXT using the utility
        current_patterns_txt = os.path.join(logs_dir, "current_detected_patterns.txt")
        convert_json_to_txt(output_data, current_patterns_txt, args.show_dates_list)
...
def main() -> None:
    ...
    try:
        ...
        try:
            # Pass target_date to get_pattern_database if args.date is provided
            pattern_db_path, version = get_pattern_database(
                project_root,
                args.pattern_version,
